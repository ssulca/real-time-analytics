...
async def orders(stream):
    async for order in stream:
        ...
        for index in range(0, len(STATUSES)):
            ...
            next_status_time = last_status_time + datetime.timedelta(
                seconds=random.randint(min, max)
            )

            other_statuses.add(
                {"id": order["id"], "updatedAt": next_status_time.isoformat(), "status": status}
            )
            last_status_time = next_status_time
        ...
        other_statuses.add(
            {"id": order["id"], "updatedAt": next_status_time.isoformat(), "status": "DELIVERED"}
        )
...
async def order_status_poller():
    statuses_to_publish = [
        item for item in other_statuses if parser.parse(item["updatedAt"]) < datetime.datetime.now()
    ]
...
async def delivery_status_poller():
    statuses_to_publish = [
        ...
        if parser.parse(item["updatedAt"]) < datetime.datetime.now()
    ]
    for status in statuses_to_publish:
