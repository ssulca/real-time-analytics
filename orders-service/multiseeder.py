...
kafkaHostPort = f"{os.getenv('KAFKA_BROKER_HOSTNAME', 'localhost')}:{os.getenv('KAFKA_BROKER_PORT', '29092')}"
...
def create_new_order(product_prices):
    ...
    for _ in range(0, number_of_items):
        ...
        items.append(
            {"productId": str(product[0]), "quantity": purchase_quantity, "price": product[1]}
        )
...
if __name__ == "__main__":
    ...
    try:
        with connect(
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    "SELECT id, name, description, category, price, image FROM pizzashop.products"
                )
                products = [
                    {
                        "id": str(row[0]),
    ...
    while True:
        order = create_new_order(product_prices)
        ...
        time.sleep(random.randint(int(orderInterval / 5), orderInterval) / 1000)
